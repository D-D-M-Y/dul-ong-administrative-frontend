...
export default function Page() {
  ...
  const validateForm = async () => {
    ...
    if (!formValues.vehicleName)
      errors.vehicleName = "Please enter the vehicle name.";
    ...
    if (!formValues.consumption)
      errors.consumption = "Please enter the vehicle's fuel consumption.";
    if (!formValues.foo)
      errors.foo = "Please enter the name of the Field Operations Officer.";
    if (!formValues.weight)
      errors.weight = "Please enter the vehicle's weight.";
    ...
    if (isValid) {
      try {
        ...
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/api/vehicles/add_vehicle`,
          {
            method: "POST",
            headers: {
  ...
  useEffect(() => {
    const fetchFooOptions = async () => {
      try {
        setLoading(true);
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/api/users/foo`
        );
        if (response.ok) {
                      ...
                      <div className="md:space-y-20">
                          ...
                          <div className="mt-2 grid grid-cols-1 gap-x-6 gap-y-3 sm:grid-cols-6">
                            ...
                            <div className="sm:col-span-3">
                              <div className="mt-2">
                                ...
                                {submitted && errors.foo && (
                                  <p className="text-red-500">{errors.foo}</p>
                                )}
                              </div>
                            </div>
                          </div>
                        ...
                        <div className="flex justify-center items-center">
                          ...
                          <button
                            ...
                            type="submit"
                            className="w-1/2 h-fit align-center bg-indigo-100 rounded-[40px] text-neutral-800 text-base font-bold font-roboto py-2 px-4"
                          >
                            Submit
                          </button>
                        </div>
